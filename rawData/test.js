...
const get_url = function () {
  const body = fs.readFileSync(path.resolve('./xo-so-truyen-thong.html'), 'utf8')
    // console.log(body);

  const $ = cheerio.load(body)
  const $body = $('body')
    // var $nav = $body.find('.sidebar-nav');
  const $leftMenu = $body.find('#left_menu')

    // find all item
  const list_url_menu = []
  let $items = $leftMenu.find('>li')
  const $today = $items.eq(0)
  $items = $items.not($today)// remove today
  $items.get().forEach(function (item) {
    const $item = $(item)
    const $a = $item.find('>a')
    const group = $a.text().trim()
    const $lis = $item.find('li')
    $lis.get().forEach(function (_item) {
      const $_item = $(_item)
      const $_a = $_item.find('a')
      list_url_menu.push({
        group: group,
        name: $_a.text().trim(),
        url: $_a.attr('href'),
...
const parse = function () {
  const body = fs.readFileSync(path.resolve('./xo-so-truyen-thong-2.html'), 'utf8')

  const $ = cheerio.load(body)
  const $body = $('body')
  const $table = $body.find('#result_mb table.kqvertimarginw')
  ...
  _.keys(lo_truc_tiep).forEach(function (item) {
    let found_dau = false, found_duoi = false
...
const clone_data = function () {
  const moment = require('moment')
  const api = new (require('./api'))()
  const global = {}
  const start_date = '04/28/2016'
  const end_date = '05/03/2016'
  api.q.when()
        .then(function () {
              ...
              const loop = function () {
                ...
                api.get(loai_id, global.start_date.format('MM/DD/YYYY'))
                            .then(function (rs) {
                              ...
                              const sql = 'INSERT INTO ketqua(giai_id, loai_id, time, value) VALUES ?'
                                // kieu 2. kieu nay phai insert tung thang 1
                              const sql2 = 'INSERT INTO ketqua SET ?'
