...
'use strict'
// ---------------------------------------------
const _httpRequest = require('./inc/httpRequest')
...
const tmp_mien = _.indexBy(mien_map, 'name')
...
api.get = function (id, ngay) {
  ...
  return self.q.when()
        ...
        .then(function () {
          ...
          const url = self.server + map_url[id].url
        ...
        .then(function (page) {
          ...
          const th_ngay = $thead.text().trim()
          const index = th_ngay.lastIndexOf(' ')
          const ngay_kq = th_ngay.substring(index).trim()
            // check date
